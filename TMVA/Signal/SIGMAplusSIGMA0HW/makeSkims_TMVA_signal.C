...
void makeSkims_TMVA_signal(){
	...
	TFile *input(0);
	TString fname = "";
	std::string label ="SIGMAplusSIGMA0HW";   

    fname = "/cms/mchristos/ANN/Seesaw/2016/92X/Signal/140/SeesawTypeIII_SIGMAplusSIGMA0HW_M-140_13TeV-madgraph/analysisTree.root";  
	input = TFile::Open(fname);
	std::cout << "--- Using input file: " << input->GetName() << std::endl;
	cout<<endl;
	float fileWeight;
	float xsec = 3.277374*0.0127815745167*0.704941513488*1.32265039022;
	...
	TFile *endfile;
	if(nleptons==4){
		endfile = new TFile("/cms/mchristos/ANN/Seesaw/2016/92X/TMVAskims/TMVA/Signal/SIGMAplusSIGMA0HW/shortTree_4L.root","RECREATE");
	}
	else if(nleptons==3){
		endfile = new TFile("/cms/mchristos/ANN/Seesaw/2016/92X/TMVAskims/TMVA/Signal/SIGMAplusSIGMA0HW/shortTree_3L.root","RECREATE");
	}
