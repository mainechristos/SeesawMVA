...
void makeSkims_TMVA_signal(){
	...
	TFile *input(0);
	TString fname = "";
	std::string label ="bash";   

    fname = "/cms/mchristos/ANN/Seesaw/2016/92X/Signal/140/SeesawTypeIII_bash_M-140_13TeV-madgraph/analysisTree.root";  
	input = TFile::Open(fname);
	std::cout << "--- Using input file: " << input->GetName() << std::endl;
	cout<<endl;
	float fileWeight;
	float xsec = run_signalSkims_TMVA.sh;
	...
	TFile *endfile;
	if(nleptons==4){
		endfile = new TFile("/cms/mchristos/ANN/Seesaw/2016/92X/TMVAskims/TMVA/Signal/bash/shortTree_4L.root","RECREATE");
	}
	else if(nleptons==3){
		endfile = new TFile("/cms/mchristos/ANN/Seesaw/2016/92X/TMVAskims/TMVA/Signal/bash/shortTree_3L.root","RECREATE");
	}
