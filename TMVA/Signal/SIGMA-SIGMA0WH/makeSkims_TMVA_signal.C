...
void makeSkims_TMVA_signal(){
	...
	TFile *input(0);
	TString fname = "";
	std::string label ="SIGMA-SIGMA0WH";   

    fname = "/cms/mchristos/ANN/Seesaw/2016/92X/Signal/140/SeesawTypeIII_SIGMA-SIGMA0WH_M-140_13TeV-madgraph/analysisTree.root";  
	input = TFile::Open(fname);
	std::cout << "--- Using input file: " << input->GetName() << std::endl;
	cout<<endl;
	float fileWeight;
	float xsec = 1.9367122*0.0128103700694*0.105965336558*1.31423243991;
	...
	TFile *endfile;
	if(nleptons==4){
		endfile = new TFile("/cms/mchristos/ANN/Seesaw/2016/92X/TMVAskims/TMVA/Signal/SIGMA-SIGMA0WH/shortTree_4L.root","RECREATE");
	}
	else if(nleptons==3){
		endfile = new TFile("/cms/mchristos/ANN/Seesaw/2016/92X/TMVAskims/TMVA/Signal/SIGMA-SIGMA0WH/shortTree_3L.root","RECREATE");
	}
