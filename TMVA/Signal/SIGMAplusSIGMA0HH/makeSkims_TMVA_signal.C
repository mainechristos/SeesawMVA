...
void makeSkims_TMVA_signal(){
	...
	TFile *input(0);
	TString fname = "";
	std::string label ="SIGMAplusSIGMA0HH";   

    fname = "/cms/mchristos/ANN/Seesaw/2016/92X/Signal/140/SeesawTypeIII_SIGMAplusSIGMA0HH_M-140_13TeV-madgraph/analysisTree.root";  
	input = TFile::Open(fname);
	std::cout << "--- Using input file: " << input->GetName() << std::endl;
	cout<<endl;
	float fileWeight;
	float xsec = 3.277374*0.000336855055297*0.421648550725*1.32265039022;
	...
	TFile *endfile;
	if(nleptons==4){
		endfile = new TFile("/cms/mchristos/ANN/Seesaw/2016/92X/TMVAskims/TMVA/Signal/SIGMAplusSIGMA0HH/shortTree_4L.root","RECREATE");
	}
	else if(nleptons==3){
		endfile = new TFile("/cms/mchristos/ANN/Seesaw/2016/92X/TMVAskims/TMVA/Signal/SIGMAplusSIGMA0HH/shortTree_3L.root","RECREATE");
	}
	...
	while (myReader.Next()) {
		...
		EventWeight = SignalWeight(Event, LS, LightLeptonTightTypeA, LightLeptonLT3, PFMETType1, LightLeptonEta0, LightLeptonEta1, LightLeptonEta2, LightLeptonEta3, LightLeptonPt0, LightLeptonPt1, LightLeptonPt2, LightLeptonPt3, PULightLeptonFlavorType, LightLeptonFlavorType3D, LightLeptonFlavorType4D,GenPileUpInteractionsTrue, LightLeptonNonFakeTypeA, LightLeptonN );
