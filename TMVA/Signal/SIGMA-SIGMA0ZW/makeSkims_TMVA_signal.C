...
void makeSkims_TMVA_signal(){
	...
	TFile *input(0);
	TString fname = "";
	std::string label ="SIGMA-SIGMA0ZW";   

    fname = "/cms/mchristos/ANN/Seesaw/2016/92X/Signal/140/SeesawTypeIII_SIGMA-SIGMA0ZW_M-140_13TeV-madgraph/analysisTree.root";  
	input = TFile::Open(fname);
	std::cout << "--- Using input file: " << input->GetName() << std::endl;
	cout<<endl;
	float fileWeight;
	float xsec = 1.9367122*0.198170900147*0.585982282439*1.31423243991;
	...
	TFile *endfile;
	if(nleptons==4){
		endfile = new TFile("/cms/mchristos/ANN/Seesaw/2016/92X/TMVAskims/TMVA/Signal/SIGMA-SIGMA0ZW/shortTree_4L.root","RECREATE");
	}
	else if(nleptons==3){
		endfile = new TFile("/cms/mchristos/ANN/Seesaw/2016/92X/TMVAskims/TMVA/Signal/SIGMA-SIGMA0ZW/shortTree_3L.root","RECREATE");
	}
